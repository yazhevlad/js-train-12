...
function sortByValues(dictionary) {
  ...
  const keysValuesArr = [...dictionary];

  keysValuesArr.sort((a, b) => b[1] - a[1]);

  const sorted = new Map(keysValuesArr);
...
function convertSetToDictionary(set) {
  ...
  for (const element of set) {
    ...
    dictionary.set(element, charCode);
  }
